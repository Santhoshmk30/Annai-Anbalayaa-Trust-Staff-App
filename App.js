...
import { ActivityIndicator, View, PermissionsAndroid, Platform, Alert,TouchableOpacity,Text,Button } from 'react-native';
...
import Completedcalls from './DashboardPage/Completedcalls/Completedcalls';
import CompletedcallsDetails from './DashboardPage/Completedcalls/CompletedcallsDetails';
import ChairmanMessage from './DashboardPage/Chairman Message/ChairmanMessage';
import CallReportDetails from './DashboardPage/CallReport/CallReportDetails'
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        if (isLoggedIn === 'true') {
          setInitialRoute('ANNAI ANBALAYAA TRUST');
        } else if (isLoggedOut === 'true') {
          setInitialRoute('SigninPage');
        } else {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Stack.Screen name="RegisterPage" component={RegisterPage} options={{ title: 'Register', headerLeft: () => null }} />
        <Stack.Screen name="VerifyOtpPage" component={VerifyOtpPage} options={{ title: 'Verify OTP' }} />
        <Stack.Screen name="ANNAI ANBALAYAA TRUST" component={Dashboard} options={{ headerShown: false }} />
        <Stack.Screen 
          name="CallDuty" 
          component={CallDuty} 
          options={({ navigation }) => ({
            title: 'Call Duty',
            headerRight: () => (
              <TouchableOpacity onPress={() => navigation.navigate('ANNAI ANBALAYAA TRUST')} style={{ paddingVertical: 6,           
    ...
    fontWeight: 'bold',  }}>Home</Text>
        ...
        <Stack.Screen name="CallReportDetails" component={CallReportDetails} options={{ title: 'Call Report Details' }} />
        ...
        <Stack.Screen name="Completedcalls" component={Completedcalls} options={{ title: 'Completed Calls' }} />
        <Stack.Screen name="CompletedcallsDetails" component={CompletedcallsDetails} options={{ title: 'Completed Calls Details' }} />
        <Stack.Screen name="ChairmanMessage" component={ChairmanMessage} options={{ title: 'Notification' }} />
      </Stack.Navigator>
    </NavigationContainer>
    </ReactNativeInactivity>
  );
